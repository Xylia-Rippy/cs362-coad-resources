...
RSpec.describe ResourceCategory, type: :model do
  ...
  describe "validation tests" do
    it "validates presence of name" do
      expect(resource).to validate_presence_of(:name)
    end

    it "validates name length" do
      expect(resource).to validate_length_of(:name).is_at_most(255)
      expect(resource).to validate_length_of(:name).is_at_least(1)
    end

    it "validates uniqueness of name" do
      ResourceCategory.create(name: "Test ResourceCategory")
  ...
  describe "member function tests" do
    ...
    it "returns the name as a string" do
      expect(resource.to_s).to eq("Test ResourceCategory")
    end
  end
